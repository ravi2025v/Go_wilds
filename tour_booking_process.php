...
// Self-healing: Ensure table has necessary column for activities
$conn->query("ALTER TABLE tour_bookings ADD COLUMN IF NOT EXISTS selected_activities TEXT AFTER total_price");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Capture Selected Activities FROM SESSION (Robust & Professional)
    $activity_total = 0;
    $activity_names = [];
    ...
    // Save total per-pax price
    $hotel_price = $basePrice + $upgrade_price;
    $totalPeople = $adults + $children;
    $total_price = ($hotel_price * $totalPeople) + 50 + (20 * ($totalPeople + $infants)) + $activity_total;

    $formatted_tour_date = date('Y-m-d', strtotime($tour_date));

    $sql = "INSERT INTO tour_bookings (tour_id, user_id, customer_name, customer_email, customer_phone, tour_date, booking_time, adults, children, infants, hotel_price, hotel_type, total_price, selected_activities, status) 
            VALUES ($tour_id, $user_id, '$customer_name', '$customer_email', '$customer_phone', '$formatted_tour_date', '$booking_time', $adults, $children, $infants, $hotel_price, '$hotel_type', $total_price, '$activities_summary', 'pending')";

    if ($conn->query($sql)) {
        // Clear activity cart after successful booking
        $_SESSION['activity_cart'] = [];
        header("Location: my-bookings.php?msg=Booking successful!");
    } else {
